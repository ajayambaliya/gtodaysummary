...
# MongoDB setup (kept functionality, removed logging)
...
def initialize_firebase():
    try:
        ...
        if service_account_json:
            ...
            logging.info("Firebase initialized with environment variable")
            return True
        ...
        if os.path.exists(path):
            ...
            firebase_admin.initialize_app(cred)
            logging.info(f"Firebase initialized with file at: {path}")
            return True
...
def send_firebase_notification(news_title, post_id):
    ...
    catchy_bodies = [
        "Unveil today’s top stories! Tap now! 📰✨",
        "Stay sharp with the latest news! Click to read! 🚀",
        "Your news fix is ready – dive in now! 🌟📢",
        "Big updates, small wait – tap now! 🔥"
    ]
    ...
    try:
        ...
        message = messaging.Message(
            ...
            data={"post_id": str(post_id), "title": news_title, "click_action": "OPEN_POST"},
            topic=topic
        )
...
def insert_news(connection, cat_id, news_title, news_description, news_image):
    ...
    try:
        logging.info(f"Attempting to insert news: {news_title}")
        ...
        logging.debug(f"Query: {query}, Data: {data}")
        return False, None
    except Exception as e:
...
class CurrentAffairsScraper:
    def __init__(self):
        ...
        if not initialize_firebase():
            logging.error("Firebase initialization failed. Notifications disabled.")
        if self.mongodb_collection is None:
            logging.warning("MongoDB unavailable. Duplicate checking disabled.")
    ...
    def safe_translate(self, text, is_title=False):
        ...
        logging.error("Translation failed after all retries")
        return False, ""
    ...
    def get_article_urls(self, page_url):
        ...
        for attempt in range(max_retries):
            try:
                logging.info(f"Fetching URLs from: {page_url}")
                ...
                logging.info(f"Found {len(urls)} URLs on {page_url}")
                return urls
                ...
                if attempt < max_retries - 1:
                    ...
                    logging.error(f"Failed to fetch URLs after {max_retries} attempts")
                    return []
            except Exception as e:
    ...
    def extract_content(self, article_urls):
        ...
        for index, article_url in enumerate(article_urls, 1):
            logging.info(f"Processing {index}/{total_urls}: {article_url}")
            try:
                ...
                if not title_success:
                    logging.error(f"Failed to translate title for {article_url}")
                    continue
                
                para_success, gujarati_paragraph = self.safe_translate(original_paragraph)
                if not para_success:
                    logging.error(f"Failed to translate paragraph for {article_url}")
                    continue
                
                articles_data.append((original_title, original_paragraph, gujarati_title, gujarati_paragraph))
                logging.info(f"Processed article {index}/{total_urls} successfully")
                time.sleep(1)
            except Exception as e:
    ...
    def format_news_content(self, articles_data):
        ...
        <div class="news-container">
            <div class="header"><h1>Daily Current Affairs Digest</h1></div>
        """
        for idx, (orig_title, orig_para, guj_title, guj_para) in enumerate(articles_data, 1):
            ...
            """
        html_content += "</div>" + self.generate_css_styles()
        return html_content
    ...
    def main(self):
        try:
            ...
            while page_number <= max_pages:
                ...
                logging.info(f"Processed page {page_number}/{max_pages}: {len(article_urls)} URLs")
                if article_urls:
            ...
            if all_urls:
                logging.info(f"Total articles to process: {len(all_urls)}")
                ...
                if articles_data and len(articles_data) >= 3:
                    ...
                    logging.info("Attempting to insert news into database")
                    success, news_id = insert_news(self.connection, self.cat_id, news_title, news_description, "")
                    
                    if success:
                        logging.info(f"Post created successfully for {current_date} with ID: {news_id}")
                    ...
                    logging.error("Not enough articles to create a post")
            else:
                logging.warning("No articles found across all pages")
...
if __name__ == "__main__":
    logging.basicConfig(
        level=logging.INFO,  # Changed to INFO for cleaner output; use DEBUG for more detail
